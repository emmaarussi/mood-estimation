...
def remove_outliers(df):
    ...
    for var in df_clean['variable'].unique():
        ...
        if var in valid_ranges:
            ...
            if n_invalid > 0:
                print(f"Removed {n_invalid} invalid values from {var} (outside [{min_val}, {max_val}])")
        ...
        if var not in ['call', 'sms']:  # Skip binary variables
            ...
            if n_outliers > 0:
                print(f"Removed {n_outliers} statistical outliers from {var}")
    ...
    for var in ['mood', 'circumplex.arousal', 'circumplex.valence']:
        ...
        if n_rapid > 0:
            print(f"Removed {n_rapid} physiologically improbable changes from {var}")
...
def handle_missing_data(df):
    ...
    # Time-based variables (screen time, app usage)
        ...
        for user_id in df_clean[df_clean['variable'] == var]['id'].unique():
            ...
            if user_data['value'].isnull().any():
                ...
                if long_gaps.any():
                    ...
                    for idx in user_data[long_gaps].index:
    ...
    for var in df_clean['variable'].unique():
        missing = df_clean[df_clean['variable'] == var]['value'].isnull().sum()
        if missing > 0:
