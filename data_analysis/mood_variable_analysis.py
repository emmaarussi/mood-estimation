...
# Constants
DATA_DIR = 'data'
OUTPUT_DIR = 'data_analysis/plots'
APP_ANALYSIS_DIR = os.path.join(OUTPUT_DIR, 'app_analysis')
...
def analyze_data_quality(df):
    ...
    plt.tight_layout()
    plt.savefig('data_analysis/plots/participation_timeline.png')
    plt.close()
    ...
    plt.ylabel('Count')
    plt.tight_layout()
    plt.savefig('data_analysis/plots/mood_sampling_intervals.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('data_analysis/plots/missing_data_patterns_by_user.png')
    plt.close()
    ...
    for var in ['call', 'sms']:
        ...
        plt.ylabel('User ID')
        plt.tight_layout()
        plt.savefig(f'data_analysis/plots/{var}_patterns_by_user.png')
        plt.close()
    
    return summary

# Import data cleaning functions
# Raw data analysis only - no cleaning
...
def plot_daily_patterns_2x2(df):
    ...
    plt.tight_layout()
    plt.savefig('data_analysis/plots/daily_patterns_2x2.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_weekly_patterns_2x2(df):
    ...
    plt.tight_layout()
    plt.savefig('data_analysis/plots/weekly_patterns_2x2.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def analyze_temporal_patterns(df):
    ...
    for var in scale_vars:
        ...
        plt.tight_layout()
        plt.savefig(f'data_analysis/plots/{var}_by_hour_per_user.png')
        plt.close()
        ...
        plt.savefig(f'data_analysis/plots/{var}_by_hour.png')
        plt.close()
        ...
        plt.savefig(f'data_analysis/plots/{var}_by_day.png')
        plt.close()
    ...
    for var in binary_vars:
        ...
        plt.savefig(f'data_analysis/plots/{var}_frequency_by_hour.png')
        plt.close()
        ...
        plt.savefig(f'data_analysis/plots/{var}_daily_distribution.png')
        plt.close()
    ...
    for var in time_vars:
        ...
        plt.savefig(f'data_analysis/plots/{var}_duration_by_hour.png')
        plt.close()
        ...
        plt.savefig(f'data_analysis/plots/{var}_daily_usage.png')
        plt.close()
...
def analyze_app_categories(df):
    ...
    plt.tight_layout()
    plt.savefig('data_analysis/plots/app_category_usage.png')
    plt.close()
    ...
    plt.ylabel('User ID')
    plt.tight_layout()
    plt.savefig('data_analysis/plots/user_app_usage_patterns.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('data_analysis/plots/app_category_correlations.png')
    plt.close()
...
def analyze_mood_correlations(df):
    ...
    plt.tight_layout()
    plt.savefig('data_analysis/plots/mood_correlations.png')
    plt.close()
    ...
    for var in significant_vars:
        ...
        plt.tight_layout()
        plt.savefig(f'data_analysis/plots/mood_vs_{var.replace(".", "_")}.png')
        plt.close()
...
def analyze_daily_patterns(df):
    ...
    plt.tight_layout()
    plt.savefig('data_analysis/plots/daily_patterns.png')
    plt.close()
...
def analyze_weekly_patterns(df):
    ...
    plt.tight_layout()
    plt.savefig('data_analysis/plots/weekly_patterns.png')
    plt.close()
...
def main():
    ...
    for dir_path in [OUTPUT_DIR, APP_ANALYSIS_DIR, TEMPORAL_PATTERNS_DIR, CORRELATIONS_DIR]:
    ...
    # 3. Analyze raw data patterns
    print("\n=== Step 3: Analyzing Raw Data Patterns ===")
    ...
    analyze_app_categories(df)
    
    # Correlations in raw data
    print("\nAnalyzing correlations in raw data...")
    correlations = analyze_mood_correlations(df)
    
    print("\nRaw data analysis complete! The following visualizations have been created in data_analysis/plots/:")
    print("\nTemporal Pattern Analysis:")
    print("1. daily_patterns_2x2.png - Daily patterns overview")
    print("2. weekly_patterns_2x2.png - Weekly patterns overview")
    print("3. [variable]_by_hour.png - Hourly patterns for each variable")
    print("4. [variable]_by_day.png - Daily patterns for each variable")
    
    print("\nApp Usage Analysis:")
    print("5. app_category_usage.png - Overall app category usage")
    print("6. user_app_usage_patterns.png - User-specific app usage patterns")
    print("7. app_category_correlations.png - Correlations between app categories")
    
    print("\nCorrelation Analysis:")
    print("8. mood_correlations.png - Raw correlations with mood")
    print("9. mood_vs_[variable].png - Detailed correlation plots")
    
    # Print top correlations with mood
    print("\nTop 5 variables most correlated with mood (raw data):")
